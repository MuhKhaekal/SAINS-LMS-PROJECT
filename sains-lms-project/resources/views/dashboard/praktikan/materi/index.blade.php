...
@section('content')
    ...
    <section class="mt-4 mx-4 md:mx-24 md:mt-24">
        <h1 class="text-center mb-6 text-xl md:text-3xl font-bold">Daftar Materi: {{ $selectedMeeting->meeting_name }}</h1>
        @forelse ($materials as $index => $material)
            <div class=" bg-white shadow-md rounded-lg p-6 border mb-4">

                <h2 class="md:block font-bold md:text-2xl border-b w-fit border-gray-300">Materi {{ $index+1 }}: {{ $material->material_name }}
                </h2>
                ...
                <div class="mt-6 w-full h-fit rounded-lg overflow-hidden border border-gray-300 md:flex justify-center">
                    ...
                    @if ($extension === 'pdf')
                        <iframe download="{{ $material->material_name . '.' . $extension }}" src="{{ $fileUrl }}"
                    ...
                    @elseif (in_array($extension, ['jpg', 'jpeg', 'png']))
                        <img src="{{ $fileUrl }}"
                            ...
                            download="{{ $material->material_name . '.' . $extension }}" />
                    ...
                    @elseif (in_array($extension, ['doc', 'docx', 'xls', 'xlsx', 'ppt', 'pptx']))
                        <div
                            ...
                            <div class="p-6 bg-white w-full max-w-md rounded-xl shadow-md border">
                                <h3 class="text-lg font-semibold text-gray-800">{{ $material->material_name }}</h3>
        ...
        @empty
            <p class="text-gray-400 italic text-center text-sm mt-12 bg-white p-8 border shadow-md rounded-md">Belum ada materi</p>
        @endforelse
    </section>
