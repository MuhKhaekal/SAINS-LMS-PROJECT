...
@section('content')
    <section class="mt-4 md:mx-24 md:mt-24">
        ...
        <div class=" bg-white shadow-md rounded-lg p-6 border ">
            ...
            <h2 class="hidden md:block font-bold text-2xl border-b w-fit border-gray-300">{{ $material->material_name }}</h2>
            ...
            <div class="mt-6 w-full h-fit rounded-lg overflow-hidden border border-gray-300 md:flex justify-center">
                ...
                @if ($extension === 'pdf')
                    <iframe  download="{{ $material->material_name . '.' . $extension }}" src="{{ $fileUrl }}" class="w-full h-[400px] md:h-[600px]" frameborder="0"></iframe>

                @elseif (in_array($extension, ['jpg', 'jpeg', 'png']))
                    <img src="{{ $fileUrl }}" class="w-full h-auto md:w-96 md:flex md:p-4 object-contain rounded-md" download="{{ $material->material_name . '.' . $extension }}" />

                @elseif (in_array($extension, ['mp3', 'wav']))
                    <div class="w-full flex justify-center items-center py-10 bg-gray-100">
                        <audio controls class="w-full max-w-xl" download="{{ $material->material_name . '.' . $extension }}">
                            <source src="{{ $fileUrl }}">
                        </audio >
                    </div>
